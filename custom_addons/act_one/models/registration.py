...
class Registration(models.Model):
    ...
    def _check_course_seat_availability(self):
        for record in self:
            if record.course_id and record.state == 'approved':
                ...
                if approved_count >= record.course_id.available_seats:
    ...
    def _check_yearly_enrollment_rules(self):
        for record in self:
            # Skip checking entirely if the record is currently in draft or rejected state
            if record.state in ['draft', 'rejected'] or not record.start_date:
                continue

            # Bypass constraint validation for the master superuser account (UID 1) to ease testing
            if self.env.uid == 1:
                continue

            # Determine target year from the course's planned starting date
            target_year = record.start_date.year
            start_of_year = date(target_year, 1, 1)
            end_of_year = date(target_year, 12, 31)

            # Look up other registrations for this employee inside the target calendar window
            ...
            # Validation 1: Block submission if a request is already stuck in review state
            ...
            # Validation 2: Block submission if a course has already been approved for this year
    ...
    def _compute_trainee_joining_date(self):
        for record in self:
            # Reaches directly into the trainee's user profile built-in creation metadata stamp
    ...
    def _check_trainee_employment_tenure(self):
        for record in self:
            ...
            # Bypasses the Master Admin (Superuser) profile for easy testing
            if self.env.uid == 1:
                continue

            # Calculate the calendar date cutoff line exactly 6 months ago from today
            six_months_ago_limit = fields.Date.today() - relativedelta(months=6)

            # Blocks the save transaction if the account was created more recently than 6 months ago
